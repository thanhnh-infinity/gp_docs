...
```
  ...
  │  + Confidence = 1.0                                  |
  |  + Ontology-Driven, manage, formalization            │
  ...
  │  + Ontology-Driven domain context (DomainVocabulary)     │
...
**On/off toggle:** `DomainVocabulary` on `SemanticaRuntimeConfig` — when `domain_vocabulary=None`, extraction works exactly as before (zero domain awareness). Currently always on in the pipeline via `_create_domain_aware_engine()` after Phase 2. To disable, skip the domain-aware engine creation.
...
| **P4** | Ontology-driven NL extraction | Medium | Domain-aware LLM extraction: better type alignment, entity grounding, novel insight capture | ✅ IMPLEMENTED — `DomainVocabulary` on `SemanticaRuntimeConfig` + custom NER/REL methods in `semantica_llm_fix.py` + Level A post-NER mapping in `semantica_engine.py` |
...
Phase  2: Structured extraction (schema-driven, deterministic)
       ↓  _build_domain_vocabulary() → DomainVocabulary → domain-aware SemanticaEngine
...
All advanced phases (5–8) are opt-in. If disabled, they are skipped with zero overhead. Domain-aware NL extraction (Phase 3) is also optional — `domain_vocabulary=None` on `SemanticaRuntimeConfig` reverts to original behavior.
